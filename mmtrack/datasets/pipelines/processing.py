...
class SingleSampling(object):
    ...
    def prepare_data(self, video_info, sampled_inds, with_label=False):
        ...
        if 'points' in video_info.keys():
            points = video_info['points']
        results = []
        for frame_ind in sampled_inds:
            if with_label:
                ...
                ann_info = dict(
                    bboxes=np.expand_dims(bboxes[frame_ind], axis=0))
                if 'points' in video_info.keys():
                    ann_info['points'] = np.expand_dims(points[frame_ind], axis=0)
            img_info = dict(
...
@PIPELINES.register_module()
class TestSingleSampling(object):
    def prepare_data(self, video_info, sampled_inds, with_label=False):
        """Prepare sampled training data according to the sampled index.
        ...
        bboxes = video_info['ann_info']['bboxes']
        if 'points' in video_info['ann_info'].keys():
            points = video_info['ann_info']['points']
        if 'noisy_bbox' in video_info['ann_info'].keys():
            noisy_bbox = video_info['ann_info']['noisy_bbox']
        results = []
        for frame_ind in sampled_inds:
            if with_label:
                ann_info = dict(
                ...
                if 'points' in video_info['ann_info'].keys():
                    ann_info['points'] = np.expand_dims(points, axis=0)
                if 'noisy_bbox' in video_info['ann_info'].keys():
                    ann_info['noisy_bbox'] = np.expand_dims(noisy_bbox, axis=0)
            img_info = dict(
                filename=video_info['img_info']['filename'],
                frame_id=video_info['img_info']['frame_id'],
                # video_id=video_info['video_id']
            )
            result = dict(img_info=img_info, ann_info=ann_info, **extra_infos)
            results.append(result)
