...
def eval_sot_ope(results, annotations, visible_infos=None):
    ...
    for single_video_results, single_video_gt_bboxes, single_video_visible in zip(  # noqa
        ...
        # add by kuiran
        single_video_gt_bboxes = single_video_gt_bboxes[:len(pred_bboxes)]
        single_video_visible = single_video_visible[:len(pred_bboxes)]
    ...
    single_sequence_success = list(np.mean(success_results, axis=1) * 100)
    single_sequence_precision = [precision_[20] * 100 for precision_ in precision_results]
    single_sequence_norm_precision = [norm_precision_[20] * 100 for norm_precision_ in norm_precision_results]
    eval_results = dict(
        success=success, norm_precision=norm_precision, precision=precision,
        single_sequence_success=single_sequence_success, single_sequence_precision=single_sequence_precision,
        single_sequence_norm_precision=single_sequence_norm_precision
    )
    return eval_results
