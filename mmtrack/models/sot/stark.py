...
class Stark(BaseSingleObjectTracker):
    ...
    def random_jitter_bbox(self, box, factor):
        w = box[2] - box[0]
        h = box[3] - box[1]
        cx = (box[0] + box[2]) / 2
        cy = (box[1] + box[3]) / 2
        import random
        cx_ = cx + w * random.uniform(-factor, factor)
        cy_ = cy + h * random.uniform(-factor, factor)
        w_ = (1 + random.uniform(-factor, factor)) * w
        h_ = (1 + random.uniform(-factor, factor)) * h
        x1_ = cx_ - w_ / 2
        y1_ = cy_ - h_ / 2
        x2_ = cx_ + w_ / 2
        y2_ = cy_ + h_ / 2
        # box[0] = box[0] + w * random.uniform(-factor, factor)
        # box[1] = box[1] + h * random.uniform(-factor, factor)
        # box[2] = box[2] + w * random.uniform(-factor, factor)
        # box[3] = box[3] + h * random.uniform(-factor, factor)
        box[0] = x1_
        box[1] = y1_
        box[2] = x2_
        box[3] = y2_
    ...
    def random_jitter_bbox_1(self, box, factor1, factor2):
        w = box[2] - box[0]
        h = box[3] - box[1]
        cx = (box[0] + box[2]) / 2
        cy = (box[1] + box[3]) / 2
        ...
        # w_ = (1 + random.uniform(-factor, factor)) * w
        # h_ = (1 + random.uniform(-factor, factor)) * h
        cx_ = cx + w * (random.choice((-1, 1)) * random.uniform(factor1, factor2))
        cy_ = cy + h * (random.choice((-1, 1)) * random.uniform(factor1, factor2))
        w_ = (1 + random.choice((-1, 1)) * random.uniform(factor1, factor2)) * w
        h_ = (1 + random.choice((-1, 1)) * random.uniform(factor1, factor2)) * h
        x1_ = cx_ - w_ / 2
        y1_ = cy_ - h_ / 2
        x2_ = cx_ + w_ / 2
        y2_ = cy_ + h_ / 2
    ...
    def simple_test(self, img, img_metas, gt_bboxes, **kwargs):
        ...
        if frame_id == 0:
            ...
            # bbox_pred = self.random_jitter_bbox(bbox_pred, self.test_cfg.random_factor)
            # bbox_pred = self.random_jitter_bbox_1(bbox_pred, self.test_cfg.random_factor, self.test_cfg.random_factor1)
            # vis
            # import cv2
            # # import numpy as np
            # import copy
            # import os
            # # vis_gt_box = gt_bboxes[0][0]
            # # vis_jitter_box =
            # prefix = 'stark_random_0.4'
            # gt_vis_1 = np.array(gt_bboxes[0][0].cpu()).astype(np.int32)
            # pred_bbox_vis = np.array(bbox_pred.cpu()).astype(np.int32)
