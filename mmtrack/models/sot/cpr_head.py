...
class GridCirclesPtFeatGenerator(GridPtFeatGenerator):
    ...
    def get_chosen_neighbours(self, pts, centers, stride):
        ...
        chosens = torch.any(dis <= self.radius * stride, dim=-1).squeeze(
            dim=-1)  # in any one of circles => ((num_gt, H, W)
...
class SinglePointExtractor(BaseModule):
    ...
    def forward(self, cls_feat, gt_r_points, gt_labels, img_metas, gt_points_ignore=None):
        """
        Args:
            cls_feat:
            ins_feat:
        ...
        """
        (pos_cls_feat,), pos_data, _, _ = self.extract(
            (cls_feat,), gt_r_points, gt_labels, img_metas, gt_points_ignore)
        pos_data.cls_feats = pos_cls_feat
        return pos_data
...
class PointRefiner(object):
    ...
    def refine_single(self, bag_data: PtAndFeat, grid_data: PtAndFeat, gt_r_points, gt_labels,
        ...
        return refine_pts, refine_scores, not_refine, chosen_pts
    ...
    def __call__(self, bag_data: PtAndFeat, grid_data: PtAndFeat, gt_r_points, gt_labels, img_metas,
        ...
        refine_pts, refine_scores, not_refine, chosen_pts = multi_apply(
        ...
        return refine_pts, refine_scores, not_refine
...
class CPRHead(AnchorFreeHead):
    ...
    def get_bboxes(self, cls_feat, ins_feat, img_metas,
        ...
        points, scores, not_refine = self.point_refiner(bag_data, grid_data, gt_r_points, gt_labels, img_metas,
