...
class SimpleEmbeddingRPNHead(BaseModule):
    def __init__(self,
                 num_proposals=100,
                 proposal_feature_channel=256,
                 init_cfg=None,
                 **kwargs):
        super(SimpleEmbeddingRPNHead, self).__init__(init_cfg)
    ...
    def init_weights(self):
        ...
        image.
        """
        super(SimpleEmbeddingRPNHead, self).init_weights()
        nn.init.constant_(self.init_proposal_bboxes.weight[:, :2], 0.5)
        nn.init.constant_(self.init_proposal_bboxes.weight[:, 2:], 1)
        self.init_proposal_bboxes.weight.requires_grad = False
...
@HEADS.register_module()
class SimpleRefineRoIHead(SparseRoIHead):
    def __init__(self, *args, **kwargs):
        super(SimpleRefineRoIHead, self).__init__(*args, **kwargs)
...
@HEADS.register_module()
class SimpleDIIHead(UTIDIIBoxHead):
    def __init__(self, bbox_coder_point=None, *args, **kwargs):
        super(SimpleDIIHead, self).__init__(*args, **kwargs)
...
@HEADS.register_module()
class SimpleFasterST2(BaseModule):
    def __init__(self, bbox_roi_extractor, bbox_head, point2box_coder, train_cfg=None, test_cfg=None, pretrained=None):
        super(SimpleFasterST2, self).__init__()
    ...
    def forward_train(self, x, proposal_list, gt_bboxes, img_metas, imgs_whwh, points=None):
        ...
        # with torch.no_grad():
        #     text_feature = self.text_linear(self.clip_model.encode_text(text).float())
        # # # add
...
@HEADS.register_module()
class SimpleFasterST2_with_pred_iou(BaseModule):
    def __init__(self, bbox_roi_extractor, bbox_head, iou_head, bbox_coder, iouloss):
        super(SimpleFasterST2_with_pred_iou, self).__init__()
        self.bbox_roi_extractor = build_roi_extractor(bbox_roi_extractor)
        self.bbox_head = build_head(bbox_head)
...
@HEADS.register_module()
class SimplePoint2Box(BaseModule):
    def __init__(self, bbox_head, point2box_coder, train_cfg=None, test_cfg=None, pretrained=None):
        super(SimplePoint2Box, self).__init__()
        self.bbox_head = build_head(bbox_head)
        self.point2box_coder = build_bbox_coder(point2box_coder)
        self.point_reg = nn.Sigmoid()
        self.train_cfg = train_cfg
    ...
    def forward_train(self, x, gt_bboxes, img_metas, imgs_whwh, points=None):
        point_feat = self.extra_point_feat(x, points)
        # use text feature
        # text_info = []
        # for img_meta in img_metas:
        #     text_info.append(img_meta['ori_filename'].split('/')[0])
        ...
        #     text_feature = self.text_linear(self.clip_model.encode_text(text).float())
        # # add
        # point_feat = point_feat + text_feature

        # concat
        # point_feat = torch.cat((point_feat, text_feature), dim=1)

        _, bbox_pred = self.bbox_head(point_feat)
        all_loss = {}
        loss_bbox = self.loss(
            bbox_pred,
            gt_bboxes,
    ...
    def simple_test(self, x, img_metas, imgs_whwh, points=None):
        points = points[0]
        point_feat = self.extra_point_feat(x, points)
        # use text_ with add
        # text_info = []
        ...
        _, bbox_pred = self.bbox_head(point_feat)
        bbox_pred = self.point_reg(bbox_pred) * imgs_whwh
        decode_bbox = self.point2box_coder.decode(points, bbox_pred)
        scale_factor = img_metas[0]['scale_factor']
        ori_pred_bbox = decode_bbox / decode_bbox.new_tensor(scale_factor)

        return dict(pred_bbox=decode_bbox, ori_pred_bbox=ori_pred_bbox)
