...
class SinglePosAnchorRPNHead1(BaseModule):
    """
    Args:
...
import random
import math
# from . import UnifiedTrackersInput


@MODELS.register_module()
class UnifiedTrackersInput1(TwoStageDetector):
    def __init__(self, *args, **kwargs):
        super(UnifiedTrackersInput1, self).__init__(*args, **kwargs)
    ...
    def simple_test(self, img, img_metas, points, gt_bboxes, noisy_bbox=None, rescale=False):
        ...
        import copy
        import os
        prefix = 'uti1/vis/lasot_36ep_1/dynamic_bbox0.3_0.4'
        # prefix = 'uti/vis/debug/lasot_24ep_1/point'
        # prefix = 'debug1'
        ...
        if input_mode == 'bbox':
            ...
            igs1 = cv2.circle(igs1, (vis_points[0], vis_points[1]), 2, (0, 0, 255), 4)
        cv2.imwrite('exp/{}/{}.jpg'.format(prefix, video_name), igs1)

        return dict(ori_pred_bbox=result['ori_pred_bbox'])


@MODELS.register_module()
class UnifiedTrackersInput2(TwoStageDetector):
    def __init__(self, *args, **kwargs):
...
@HEADS.register_module()
class UnifiedTrackersInputRoIHead1(CascadeRoIHead):
    def __init__(self,
                 num_stages=6,
    ...
    """
    faster refine+select + sparse refine
    """
    # def forward_train(self,
    #                   x,
    #                   points,
    ...
    #     all_stage_loss = {}
    #     # select
    #     # pred_ious, pred_bbox, bbox_loss, pred_iou_loss = self.pred_iou_head(x, proposal_list, gt_bboxes, img_metas, imgs_whwh)
    #     pred_bbox, max_ious_index, all_loss = self.pred_iou_head(x, proposal_list, gt_bboxes, img_metas,
    #                                                                         imgs_whwh)
    #     all_stage_loss.update(all_loss)
    #
    #     # rois = bbox2roi(proposal_list)
    #     # pred_ious = self._iou_forward(points, 0, x, rois, pred_iou_proposal_features, img_metas, imgs_whwh)
    #     # pred_ious_loss = self.pred_iou_head.loss(proposal_boxes, gt_bboxes, pred_ious, imgs_whwh=imgs_whwh)
    #     # all_stage_loss['loss_pred_ious'] = pred_ious_loss['loss_pred_ious']
    #     # max_pred_ious, max_ious_index = torch.max(pred_ious, dim=1)
    #
    #     # select proposal feat
    #     # _of = []
    #     # # _of.append(box_proposal_features[i, max_ious_index[i][0].item(), :])
    #     # if input_mode == 'point':
    #     #     for i in range(num_imgs):
    #     #         _of.append(point_proposal_features[i, max_ious_index[i].item(), :])
    #     # elif input_mode == 'bbox':
    #     #     for i in range(num_imgs):
    #     #         _of.append(box_proposal_features[i, max_ious_index[i].item(), :])
    #     # object_feats = torch.stack(_of).unsqueeze(1)
    #
    #     # get text_info replace proposal_feat
    #     text_info = []
    ...
    #     # for i in range(num_imgs):
    #     #     _pl.append(proposal_list[i][max_ious_index[i][0].item(), :].unsqueeze(0))
    #     proposal_list = pred_bbox
    #     # cal iou
    #     # proposal_bbox_ious = torch.cat(_pl)
    #     # gt_bboxes_iou = torch.cat(gt_bboxes)
    #     # pro_ious = bbox_overlaps(proposal_bbox_ious, gt_bboxes_iou, is_aligned=True)
    #     # all_stage_loss['proposal_gt_ious'] = pro_ious
    #     # all_stage_loss['proposal_pred_ious'] = max_pred_ious
    #
    #     for stage in range(self.num_stages):
    #         rois = bbox2roi(proposal_list)
    #         bbox_results = self._bbox_forward(points, stage, x, rois, object_feats,
    ...
    #     # max_pred_ious, max_ious_index = torch.max(pred_ious_stage6, dim=1)
    #     # max_ious_index = torch.argmax(pred_ious_stage6, dim=1)
    #     pred_bboxes_stage6 = all_stage_bbox_results[5]['decode_bbox_pred'].reshape(2, -1, 4)
    #     # bbox_ious = []
    #     # for i in range(max_ious_index.shape[0]):
    #     #     bbox_ious.append(pred_bboxes_stage6[i, max_ious_index[i, :][0], :])
    #     bbox_ious = pred_bboxes_stage6
    ...
    def simple_test(self, x, points, proposal_boxes, point_proposal_features, bbox_proposal_features, pred_iou_proposal_features,
        ...
        if self.custom_cfg.use_text_feat:
            ...
            # if text_info[0] == 'a licenseplate':
            #     text_info[0] = 'a car'
            # elif text_info[0] == 'a bicycle':
            #     text_info[0] = 'a person'
            # elif text_info[0] == 'a guitar':
            #     text_info[0] = 'a person'
            # elif text_info[0] == 'a hand':
            #     text_info[0] = 'a person'
            # elif text_info[0] == 'a hat':
            #     text_info[0] = 'a person'
            # elif text_info[0] == 'a helmet':
            #     text_info[0] = 'a person'
            # elif text_info[0] == 'a microphone':
            #     text_info[0] = 'a person'
            # elif text_info[0] == 'a yoyo':
            #     text_info[0] = 'a person'
            # elif text_info[0] == 'a basketball':
            #     text_info[0] = 'a person'
            # elif text_info[0] == 'a motorcycle':
            #     text_info[0] = 'a person'

            # if text_info[0] == 'cattle':
            #     text_info[0] = 'one cattle'
